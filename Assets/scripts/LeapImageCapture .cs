/*
LeapImageCapture.cs - A script to capture images from the Leap Motion camera and save them as PNG files.
This script uses the LeapC library to capture images from the Leap Motion camera. It sets the policy flags to allow
images, polls the connection for events, and saves the images as PNG files when available. The saved images are stored
in the user's Documents folder in subfolders named 'left' and 'right'.
Requirements:
Leap Motion SDK (https://developer.leapmotion.com/sdk-leap-motion-controller/)
LeapCSharp (https://developer.leapmotion.com/sdk-leap-motion-controller/)
Usage:
Attach the script to a GameObject in your Unity scene.
Run the scene.
Left and right images will be saved as PNG files in the user's Documents folder in subfolders named 'left' and 'right'.
Note:
This script is intended for use with the Leap Motion Controller, and will not work without one.
The saved images may be blurry or low quality depending on lighting and other environmental factors.
*/
...
/// <summary>
/// A class that captures images from a Leap Motion device and saves them as PNG files to disk.
/// </summary>
public class LeapImageCapture : MonoBehaviour{
    ...
    // Create a configuration object for the Leap Motion connection
    ...
    // Create and open a connection to the Leap Motion service
    private void Start() 
    {   
    ...
    // Connect to the Leap Motion image event and retrieve image data
    private void Update() 
    {
        // RetrieveDeviceList(hConnection);
        
        // Call ConnectImageEvent coroutine to connect to the image event and retrieve image data
    ...
    private void OnDestroy() {
        ...
        LeapC.ClearConnection(hConnection);
